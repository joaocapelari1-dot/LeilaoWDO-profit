...
function MercadoMundialPanel({ macro, tick, dolFeatures }) {
  ...
  const ativos = [
    ...
    { label:'OURO',      val: macro?.gold?.price,        chg: macro?.gold?.changePct,      inv: false   },
    ...
    <div style={{ minHeight:'100vh', background:'#080c10', color:'#e2e8f0', fontFamily:'monospace' }}>
      ...
      <div style={{ borderBottom:'1px solid #1e2832', padding:'6px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', background:'#0c1219' }}>
        ...
        <div style={{ display:'flex', gap:6, alignItems:'center' }}>
          ...
          {['dashboard','execution','risk','status'].map(t => (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'220px 1fr 560px', gap:1, padding:1, height:'calc(100vh - 39px)', overflow:'hidden' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:1, overflow:'hidden', height:'100%' }}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, flex:1, overflow:'hidden' }}>
              {/* Durante leilão (8h55-9h05): Mapa de Força em vez do SuperDOM */}
              {(() => {
                const hora = new Date().getHours() + new Date().getMinutes()/60
                const emLeilao = hora >= 8.917 && hora <= 9.083
                return emLeilao ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <MapaForca features={socket.features} />
                    <div style={{ color: '#64748b', fontSize: 9, textAlign: 'center' }}>
                      SuperDOM indisponível durante leilão — usando Tape + Icebergs
                    </div>
                  </div>
                ) : (
                  <>
                    <SuperDOM book={socket.book} features={socket.features} levels={80} />
                    <SuperDOMDOL dolFeatures={socket.dolFeatures} levels={80} />
                  </>
                )
              })()}
            </div>
      ...
      {tab === 'risk' && (
        <div style={{ padding:16 }}>
          <div style={{ background:'#0c1219', border:'1px solid #1e2832', borderRadius:6, padding:20 }}>
            <h2 style={{ fontSize:12, letterSpacing:2, color:'#64748b', marginBottom:12 }}>RISK ENGINE</h2>
            {socket.riskEvent
              ? <pre style={{ fontSize:10, color:'#94a3b8' }}>{JSON.stringify(socket.riskEvent, null, 2)}</pre>
              : <div style={{ color:'#475569', fontSize:11 }}>Aguardando sinais...</div>}
          </div>
        </div>
      )}

      
      {tab === 'status' && <APIStatus connected={socket.connected} aiAnalysis={socket.aiAnalysis} tick={socket.tick} adaptive={socket.adaptive} />}
    </div>
  )
}
// updated Ter  9 Jun 2026 17:01:25 -03
